...
if __name__ == '__main__':
    ...
    for i in range(0, len(list[0][2])):
        ...
        if os.path.exists('right/' + str(a[i])):
            img1 = cv2.imread('right/' + str(a[i]))
            img1 = img1[260:340, 1010:1110]
        else:
            continue
        for j in range(i + 1, len(list[0][2])-1):

            if os.path.exists('right/' + str(a[j])):
                img2 = cv2.imread('right/' + str(a[j]))
                img2 = img2[260:340, 1010:1110]
            else:
                continue
            degree = classify_aHash(img1, img2)
            if degree <= 7:
                 os.remove('right/' + str(a[j]))
                 print(i, j)
